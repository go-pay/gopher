...
// Map is like a Go map[comparable]any but is safe for concurrent use
...
func (m *Map[K, V]) Store(key K, value V) {
	read := m.loadReadOnly()
	if e, ok := read.m[key]; ok && entryTryStore(e, &value) {
		return
	}
...
// entryTryStore stores a value if the entry has not been expunged.
//
// If the entry is expunged, entryTryStore returns false and leaves the entry
// unchanged.
func entryTryStore[V any](e *entry, i *V) bool {
	for {
		p := atomic.LoadPointer(&e.p)
		if p == expunged {
			return false
