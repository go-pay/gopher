...
func ParseDateTime(timeStr string) (datetime time.Time) {
	datetime, _ = time.ParseInLocation(goutil.TimeLayout, timeStr, time.Local)
	return
}
...
func ParseDate(timeStr string) (date time.Time) {
	date, _ = time.ParseInLocation(goutil.DateLayout, timeStr, time.Local)
	return
}
