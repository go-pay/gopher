...
func NewV2(code int, reason string, message string) *ErrorV2 {
...
func FromError(err error) *ErrorV2 {
	...
	if !ok {
		return NewV2(UnknownCode, UnknownReason, err.Error())
	}
	ret := NewV2(DefaultConverter.FromGRPCCode(gs.Code()), UnknownReason, gs.Message())
...
func errStringToErrorV2(e string) *ErrorV2 {
	...
	if err != nil {
		return NewV2(-1, UnknownReason, e)
	}
	return NewV2(i, UnknownReason, e)
}
